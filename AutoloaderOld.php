...
class Autoloader {
	...
	public static function load($targetClass) {
		...
		$backtraceIndexFile = isset($backtrace[1]['file']) ? 1 : 2;
		$backtraceIndexClass = isset($backtrace[2]) ? 2 : 1;
		$partsFileCalledFrom = explode(DIRECTORY_SEPARATOR, dirname($backtrace[$backtraceIndexFile]['file']));
		$partsClassCalledFrom = array();

		if (isset($backtrace[$backtraceIndexClass]['class'])) {
			$partsClassCalledFrom = explode('\\', $backtrace[$backtraceIndexClass]['class']);
		}
		...
		if (file_exists($path)) {
			foreach (glob(dirname($path).'/*.php') as $filename) {
				include $filename;
			}
			require_once $path;
